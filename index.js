...
async function run() {
    try {
        ...
        app.post('/jwt', (req, res) => {
            const user = req.body;
            const token = jwt.sign(user, process.env.ACCES_TOKEN_SECURITY, { expiresIn: '3h' });
            res
                .cookie('token', token, {
                    httpOnly: true,
                    secure: false,
        ...
        //flashsales
        const flashsalesCollection = client.db("easyBazzar").collection("flashsales")
        ...
        app.delete('/product/delete/:id', verifyToken, adminVerify, async (req, res) => {
            const id = req.params.id;
            const query = { _id: new ObjectId(id) };
        ...
        //brandsproducts
        const brandsproductsCollection = client.db("easyBazzar").collection("brandsProducts")
        app.get('/brandsProducts/:category_id', async (req, res) => {
            const category_id = req.params.category_id;
            const query = {
                category_id: category_id
            }
            const result = await brandsproductsCollection.find(query).toArray();
            res.send(result)
        })
        ...
        app.get('/brandproduct/:name', async (req, res) => {
            ...
            const result = await brandsproductsCollection.findOne(query);
            console.log(result)
            res.send(result);
        })
